...
def main():
    ...
    curr = copy.deepcopy(test._netgraph)
    for peer_id in curr:
        ce = curr[peer_id]
        if ce.edge_type == "CETypeLongDistance":
            curr.remove_conn_edge(peer_id)
            break
    ce = ConnectionEdge("4eh67", edge_type="CETypeLongDistance")
    curr.add_conn_edge(ce)
    curr[ce.peer_id] = ce
    net_op = NetworkOperations(test._netgraph, curr)
    NetworkOperations.find_Difference(net_op)
    for i in net_op:
        print(i)
